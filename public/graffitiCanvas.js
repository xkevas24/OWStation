 ...
 *  @author eguid
 */
GraffitiCanvas=(function(){
	//公共方法
	var getDom=function(id){return document.getElementById(id)},//获取dom
  ...
  var tmp;
	...
	var drawSnapshot=function(){
		...
		// console.log("绘制快照");
    cxt.putImageData(canvasHistory[step],0,0);
		//cxt.putImageData(drawnSnapshot,0,0);
		return true;
	}

  var recordNow=function(){
	  step++;
    if (step < canvasHistory.length) {
      canvasHistory.length = step; // 截断数组
    }
    canvasHistory.push(cxt.getImageData(0,0,cbtCanvas.width,cbtCanvas.height))
    // console.log('写入步骤')
    return true
  };
	...
	return{
      ...
      switch(ctrlConfig.kind){
        ...
        case graphkind.trapezoid://梯形
          ...
          break;
        case graphkind.circle://圆
          //console.log("确定图形绘制开始坐标点："+x+","+y);//点击确定图形的开始坐标点
          beginDrawing();//开始绘制
          var p=new Point(x,y);
          setStartPoint(p);
          var circle= new Circle({'start':p});
    ...
    touchMove: function(x,y){
      ...
      if(isDrawing()&&hasStartPoint()){//检查是否开始绘制，检查是否有开始坐标点
        ...
        var p=setCuPointXY(x,y,0);//设置共享的临时坐标点，用于防止重复创建对象
        switch(ctrlConfig.kind){
          case graphkind.pen://画笔(一直画)
            cxt.lineTo(x,y);
            cxt.stroke();
		...
		setPaintConfig:function(params){
			...
			paintConfig.lineWidth=isNull(params.lineWidth)?3:params.lineWidth,//线条宽度，默认1
			paintConfig.strokeStyle=isNull(params.strokeStyle)?'#FFFF00':params.strokeStyle,//画笔颜色，默认红色
			paintConfig.fillStyle=isNull(params.fillStyle)?'#FFFF00':params.fillStyle,//填充色
    ...
    PutHero:function (hero){
		  //console.log('添加英雄')
      var old_md = cbtCanvas.onmousedown
      var old_mv = cbtCanvas.onmousemove
      var old_mp = cbtCanvas.onmouseup
      let img = new Image();
      img.src="heroes/"+hero+".png";
      // 保存以前的画布
      tmp = cxt.getImageData(0, 0, cbtCanvas.width,cbtCanvas.height);
      img.onload=function () {
        cxt.drawImage(img,100,100,45,45);
      };
      cbtCanvas.onmousedown=function (e) {
        //按下后可移动
        cbtCanvas.onmousemove = function(e){
          x= e.clientX;y=e.clientY;

          cxt.putImageData(tmp, 0, 0);
          cxt.drawImage(img,x-75,y-50,45,45);
        };
        //鼠标抬起清除绑定事件
        cbtCanvas.onmouseup = function(){
          step++;
          if (step < canvasHistory.length) {
            canvasHistory.length = step; // 截断数组
          }
          canvasHistory.push(document.querySelector('#canvas').toDataURL())
          cbtCanvas.onmousedown = old_md
          cbtCanvas.onmousemove = old_mv
          cbtCanvas.onmouseup = old_mp
        };
      }
    }
	}
