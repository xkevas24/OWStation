...
<template>
  <q-layout view="hHh lpR fFf">
    ...
    <q-page-container @mouseenter="hideProgress">

      <q-page style="background-color: #282828">
        <div class="pause" id="pausebtn">
          <q-icon name="play_circle_filled" size="50px" color="white" @click="VPP"/>
        </div>
        <q-drawer
          side="right"
          show-if-above
        ...
        <div id="toolBar" class="toolbar">
          <q-toolbar>
            <q-toolbar-title style="text-align: center">
              <q-btn flat color="white" icon="replay_5" size="25px" id="btn_back" @click="PlayerBefore">
                <q-tooltip content-class="bg-blue-4">快退5秒，快捷键：I</q-tooltip>
              </q-btn>

              <q-btn flat color="white" :icon="ppicon" @click="VPP" id="btn_pp" size="25px">
                <q-tooltip content-class="bg-blue-4">播放/暂停，快捷键：O</q-tooltip>
              </q-btn>

              <q-btn flat color="white" icon="forward_5" size="25px" id="btn_next" @click="PlayerNext">
                <q-tooltip content-class="bg-blue-4">快进5秒，快捷键：P</q-tooltip>
              </q-btn>
              ...
              <q-btn flat color="white" icon="img:toolbar/hero-txt.png" size="25px" id="btn_hero" style="display: none" @click="heroActive">
    ...
    <q-footer id="footer" class="bg-dark" @mousehover="showProgress" @mousemove="showProgress" @mousedown="showProgress" @mouseenter="showProgress" >
      <q-linear-progress :value="video_progress" color="grey-3" style="display: block;position: fixed;bottom: 10px" id="progress"/> <!--  indeterminate -->
      <q-slider :value="video_slider" @change="val => {video_jump = val}" :min="0" :max="video_seconds" color="grey-3" style="display: none;position: fixed;bottom: -8px" id="slider"/>
    </q-footer>
  </q-layout>
</template>
...
<script>
var teamListener;
var videoSourceListener;
var videoProcessListener;
var touchListener;
var tailListener;
var herocss=''
var paintcss=''
...
export default {
  ...
  watch:{
    video_progress(val,old_val){
      //console.log(val)
    },
    video_jump(val){
      console.log(val)
      document.getElementById('MainPlayer').currentTime = val
      this.hideProgress()
    },
    blue1(val){
  ...
  methods:{
    teamListen(){
      ...
      if(this.$q.electron.remote.getCurrentWindow().isFullScreen()){
    ...
    videoSource(){
      var saveSource = this.$q.localStorage.getItem('videoSrc')
      var currentSource = decodeURI(document.getElementById('MainPlayer').src).replace('file:///','').replace(/\//g,'\\')
      if(currentSource !== saveSource && saveSource !== null) {
        console.log('修改视频播放源，旧：' + saveSource + '，新：' + currentSource)
        document.getElementById('MainPlayer').src= saveSource
        // 初始化视频播放位置
        var vp = this.$q.localStorage.getItem(document.getElementById('MainPlayer').src)
        console.log(document.getElementById('MainPlayer').src + '===' + vp)
        if (vp !== null) {
          document.getElementById('MainPlayer').currentTime = vp
        }
      }
    },
    windowClose(){
    ...
    PlayerPause () {
      ...
      this.ppicon = 'play_arrow'
      // this.$q.sessionStorage.set('pp', 'pause')
    },
    PlayerPlay () {
      document.getElementById('MainPlayer').play()
      this.ppicon = 'pause'
      // this.$q.sessionStorage.set('pp', 'play')
    },
    VPP () {
      this.allBack()
      if (document.getElementById('MainPlayer').paused){
        this.PlayerPlay()
        document.getElementById('pausebtn').className = 'pause ele-hide'
      }else {
        this.PlayerPause()
        document.getElementById('pausebtn').className = 'pause ele-show'
      }
    },
    PlayerNext () {
      this.allBack()
      // 获取当前位置 currentTime
      var ct = document.getElementById('MainPlayer').currentTime
      document.getElementById('MainPlayer').currentTime = ct + 5
    },
    PlayerBefore () {
      this.allBack()
      // 获取当前位置 currentTime
      ...
      document.getElementById('MainPlayer').currentTime = ct_then
    },
    videoProcessListen () {
      var video = document.getElementById('MainPlayer')
      var vl = Math.ceil(video.duration);
      this.video_seconds = vl
      var vp = Math.ceil(video.currentTime)
      this.video_slider = vp
      var vr = vp/vl
      this.video_progress = vr
      // 记录播放位置
      if (vp !== 0) {
        this.$q.localStorage.set(document.getElementById('MainPlayer').src,vp)
        //console.log('记录进度：' + vp)
      }
    },
    Pen (pen) {
  ...
  mounted() {
    ...
    videoSourceListener = setInterval(this.videoSource,200)
    var lang = this.$q.localStorage.getItem('lang')
    // 初始化系统组件语言
    this.g_title = this.sys[lang].title
    this.btn_setting = this.sys[lang].btn_setting
    ...
    // 初始化视频宽度
    var video_wd = cw - 66
    document.getElementById('MainPlayer').style.width = video_wd + "px"
    // 初始化视频进度条
    videoProcessListener = setInterval(this.videoProcessListen,10)
    // 初始化播放暂停位置 pausebtn
    console.log(document.body.clientHeight - 80 +"px")
    document.getElementById('pausebtn').style.top = document.body.clientHeight - 160 +"px"
    document.getElementById('pausebtn').style.left = "20px"
    // 初始化画布
    var canvas_wd = document.getElementById('MainPlayer').offsetWidth
    var canvas_ht = document.getElementById('MainPlayer').offsetHeight + 100
    ...
    document.getElementById('MainPlayer').volume = 0.2
    this.Pen('pencil')
  }
}
</script>
